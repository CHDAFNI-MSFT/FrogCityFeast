...
|---|---|---|---|
| `icon.svg` | Original Copilot-assisted frog-and-city SVG authored for this repository in September 2026. It follows the project's production frog, storybook palette, and appetite theme without third-party artwork or trademarks. | Project-specific copyright belongs to the repository owner. The repository has no general open-source license; reuse outside the project requires permission. | The committed 1024-by-1024, full-bleed SVG is the editable source. Godot 4.7.2 import regenerates `icon.svg.import`; the reviewed iOS export workflow generates the platform AppIcon derivatives from the same source. |
